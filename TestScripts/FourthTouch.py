...
async def findUsername(username, interfaceType):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        userFile.close()
        df = pd.json_normalize(userJson['sites'])
        csvPathSave = os.path.join(path, 'results', username + '.csv')
        df.to_csv(csvPathSave, encoding='utf-8', index=False)
        print(f"{Fore.LIGHTYELLOW_EX}[!] Search complete in {executionTime} seconds\033[0m")
        print(f"{Fore.LIGHTYELLOW_EX}[!] Results saved to {username}.json and {username}.csv\033[0m")
        return userJson
    ...
    try:
        async with session.request(u["method"], url, json=jsonBody,proxy=proxy, headers=headers, ssl=False) as response:
            ...
            if eval(u["valid"]):
                ...
                if 'metadata' in u:
                    ...
                    for d in u["metadata"]:
                        try:
                            ...
                            pass
                return ({"id": u["id"], "name": u['app'], "url_user": url, "http_status": f"{response.status}", "exists": "FOUND", "error-message": None, "metadata": metadata})
            else:
                ...
                return ({"id": u["id"], "name": u['app'], "url_user": url, "http_status": f"{response.status}", "exists": "NOT FOUND", "error-message": None, "metadata": metadata})
    except Exception as e:
        if interfaceType == 'CLI':
            if showAll:
                print(f'{Fore.RED}[X]\033[0m - #{u["id"]} {Fore.BLUE}{u["app"]}\033[0m error on request ({repr(e)})- {Fore.YELLOW}{url}\033[0m')
        return ({"id": u["id"], "name": u['app'], "url_user": url, "http_status": None, "exists": "ERROR", "error-message": repr(e), "metadata": metadata})
...
if __name__ == '__main__':
    ...
    ░          ░  ░     ░  ░░ ░      ░  ░    ░       ░     ░        ░    
        ░                  ░                     ░               ░      
                                        ...
                                        Made with ❤ by """ + Fore.BLUE + "p1ngul1n0\n")
